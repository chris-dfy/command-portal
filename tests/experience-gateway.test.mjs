...
test("every Experience Gateway route maps to exactly one literal runtime endpoint", async () => {
  ...
  for (const [gatewayPath, runtimePath] of Object.entries(RUNTIME_ROUTES)) {
    ...
    if (alias?.forwarding === "unavailable_adapter") {
  ...
  const unavailableReadCount = Object.keys(RUNTIME_ROUTES)
    .filter((gatewayPath) => resolveGatewayRuntimeActionAlias("GET", gatewayPath)?.forwarding === "unavailable_adapter")
    .length;
  assert.equal(observed.length, Object.keys(RUNTIME_ROUTES).length - unavailableReadCount);
});
...
test("unavailable Realtime WebRTC never crosses the same-origin gateway", async () => {
  ...
  assert.equal(body.error.code, COMMAND_PORTAL_REALTIME_QUARANTINE_CODE);
  assert.equal(body.voice.profile, COMMAND_PORTAL_REALTIME_PROFILE);
  assert.equal(body.voice.provesFullDuplexReadiness, COMMAND_PORTAL_PROVES_FULL_DUPLEX_READINESS);
  assert.equal(body.voice.connectionState, "unavailable");
  assert.equal(response.headers.get("x-request-id"), body.error.requestId);
  assert.match(body.error.requestId, /^[0-9a-f-]{36}$/);
  assert.equal(calls, 0);
});

test("even a live-verified Runtime projection cannot override the quarantined Experience adapter", async () => {
  const offer = "v=0\r\na=offer\r\n".repeat(12);
  const projection = actionAdmissionProjection({
    classifications: {
  ...
  const response = await fetch(`${base}/api/runtime/realtime/call`, {
    ...
    headers: {
      ...
      "X-Request-ID": "voice-quarantine-test-1",
    },
    body: offer,
  });
  const body = await response.json();
  assert.equal(response.status, 503);
  assert.equal(body.error.code, COMMAND_PORTAL_REALTIME_QUARANTINE_CODE);
  assert.equal(body.error.requestId, "voice-quarantine-test-1");
  assert.equal(response.headers.get("x-request-id"), "voice-quarantine-test-1");
  assert.equal(body.voice.activationAllowed, false);
  assert.equal(body.voice.provesFullDuplexReadiness, false);
...
test("Realtime gateway rejects unavailable actions and unsafe methods before contacting Runtime", async () => {
  ...
  assert.equal((await invalid.json()).error.code, COMMAND_PORTAL_REALTIME_QUARANTINE_CODE);
...
test("hosted canonical interaction requires session, CSRF, and exact idempotency while unavailable Realtime remains blocked", async () => {
  ...
  assert.equal((await realtime.json()).error.code, COMMAND_PORTAL_REALTIME_QUARANTINE_CODE);
