...
test("Command Portal voice is registered as continuity-only and cannot prove full-duplex readiness", async () => {
  const document = await registry();
  const voice = document.surfaces.find((surface) => surface.id === "voice");
  const operator = voice?.modules.find((module) => module.moduleId === "voice.operator");
  assert.equal(voice?.clients.web.state, "functional");
  assert.equal(voice?.clients.web.voiceProfile, "continuity_only");
  assert.equal(voice?.clients.web.provesFullDuplexReadiness, false);
  assert.match(voice?.clients.web.reason ?? "", /full-duplex Realtime is quarantined/);
  assert.equal(operator?.clients.web.voiceProfile, "continuity_only");
  assert.equal(operator?.clients.web.provesFullDuplexReadiness, false);
});
