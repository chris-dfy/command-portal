...
test("local-first workspaces delegate operational truth while Command Portal voice remains continuity-only", async () => {
  ...
  assert.match(voice, /COMMAND_PORTAL_REALTIME_QUARANTINE_MESSAGE/);
  assert.match(voice, /Continuity only/);
  assert.match(voice, /Full-duplex quarantined/);
...
test("NEXUS remains a Runtime-governed conversational copilot across every portal area", async () => {
  ...
  assert.doesNotMatch(copilot, /RealtimeVoiceClient|Start live|Voice ready/);
  ...
  assert.match(copilot, /Ask by voice/);
  assert.match(admission, /The sole browser admission path for both text and finalized voice input/);
  assert.doesNotMatch(admission, /hifClient|routeTranscript|executiveIntentEndpointIsAbsent|voiceOperatorEndpointIsAbsent/);
  assert.match(realtime, /RTCPeerConnection/);
...
test("copilot and voice controls keep continuity separate from quarantined full-duplex activation", async () => {
  ...
  assert.match(copilot, /COMMAND_PORTAL_REALTIME_QUARANTINE_MESSAGE/);
  assert.doesNotMatch(copilot, /fetch\("\/api\/runtime\/realtime-voice"|Start live|RealtimeVoiceClient/);
  assert.match(voice, /Full-duplex quarantined/);
  assert.match(voice, /<button className="voice-start" disabled/);
  assert.doesNotMatch(voice, /client\.connect\(|Start live|RealtimeVoiceClient/);
  assert.match(
    voice,
