...
export function NexusCopilot({ activeArea, activeLabel, runtimeState, onNavigate, interactionAction, realtimeAction, open, expanded, onOpenChange, onExpandedChange }: {
  ...
  const voiceReady = voiceAvailable && realtimeAction.available;
  ...
  return <>
    <aside id="nexus-copilot" className={`nexus-copilot${expanded ? " is-expanded" : ""}`} aria-label="NEXUS executive copilot">
      ...
      <div className="nexus-copilot__voice">
        <div><NexusAvatar state={avatarState} amplitude={amplitude} size="xs" micMuted={microphoneMuted && voiceConnected} unavailable={!voiceReady && !voiceConnected} label={voiceConnected ? NEXUS_AVATAR_STATE_LABELS[avatarState] : voiceReady ? "Voice ready" : "Voice unavailable"} /><strong>{voiceConnected ? microphoneMuted ? "Microphone muted" : voiceState : voiceReady ? "Voice ready" : "Voice unavailable — Runtime voice cannot be established"}</strong></div>
