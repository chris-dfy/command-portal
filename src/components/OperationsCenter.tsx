...
export function OperationsCenter({ assessment }: { assessment: EoxAssessment | null }) {
  ...
  return <section className="operations-center" aria-labelledby="operations-center-heading">
    ...
    <div className="experience-grid eox-grid">
      ...
      <DataPanel eyebrow="What NEXUS can establish" title="Operational Understanding" icon={<Brain size={18} />} className="span-2"><section id="operational-understanding" className="eox-indicators understanding-grid">{assessment.operationalUnderstanding.map((item) => <article key={item.name}><header><span>{item.name}</span><StatusPill value={item.status} /></header><strong>{item.value}</strong><small>{item.basis}</small></article>)}</section></DataPanel>
