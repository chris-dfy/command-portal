...
export const COMMAND_PORTAL_REALTIME_PROFILE = "continuity_only";
export const COMMAND_PORTAL_PROVES_FULL_DUPLEX_READINESS = false;
export const COMMAND_PORTAL_REALTIME_QUARANTINE_CODE = "realtime_voice_quarantined";
export const COMMAND_PORTAL_REALTIME_QUARANTINE_MESSAGE =
  "Full-duplex Realtime voice is quarantined for this Experience adapter until its exact deployed artifact has a current authenticated audio canary receipt.";
...
export const FIXED_RUNTIME_ACTION_ALIASES = Object.freeze({
  ...
  "/api/runtime/realtime-voice": Object.freeze({
    GET: runtimeActionAlias(
      "context.runtime.route.get.runtime.voice_realtime_status",
      "GET",
      "/runtime/voice/realtime/status",
      {
        requiredSurfaces: ["api", "ui", "voice"],
        forwarding: "unavailable_adapter",
        limitation: COMMAND_PORTAL_REALTIME_QUARANTINE_MESSAGE,
      },
    ),
  }),
  ...
  "/api/runtime/realtime/call": Object.freeze({
    POST: runtimeActionAlias(
      "context.runtime.route.post.runtime.voice.realtime.call",
      "POST",
      "/runtime/voice/realtime/call",
      {
        requiredSurfaces: ["api", "voice"],
        forwarding: "unavailable_adapter",
        limitation: COMMAND_PORTAL_REALTIME_QUARANTINE_MESSAGE,
      },
    ),
...
function validateLocalPayload(runtimePath, payload, maximumBytes) {
  ...
  if (runtimePath === "/work-sessions/start") {
...
async function fetchLocalCapability(resolved, payload, request, config, localFetch) {
  ...
  const timeoutMs = resolved.runtimePath === "/executive/interactions"
  ...
  try {
    ...
    try {
      ...
      const canonicalIdentityHeaders = resolved.runtimePath.startsWith("/executive/interactions") ? {
        ...
        "X-NEXUS-Scopes": [...new Set(config.operationalScopes)].sort().join(","),
      } : {};
...
async function fetchOperationalCapability(resolved, payload, claims, request, config, operationalFetch) {
  ...
  if (
    ...
    && (
      resolved.runtimePath.startsWith("/executive/interactions")
  ...
  try {
    ...
    const authoritativeInteraction = resolved.runtimePath.startsWith("/executive/interactions")
      || /^\/approvals\/.+\/(approve|deny)$/.test(resolved.runtimePath);
...
async function handleOperationalApi(
  ...
  try {
    ...
    } else if (resolved.method === "POST" && resolved.runtimePath === "/executive/interactions") {
      rejectUntrustedOperationalFields(rawPayload, [], new Set(["approval_id"]));
      payload = validateLocalPayload(
        resolved.runtimePath,
        rawPayload,
        config.localMaxRequestBytes,
...
function resolveCanonicalExecutionRoute(pathname, method) {
  ...
  const match = pathname.match(
    /^\/api\/canonical-execution\/missions\/([^/]+)$/,
  );
  if (!match) return null;
  const missionId = operationalIdentifier(match[1]);
  if (!missionId) return null;
  if (method !== "GET") {
    return { methodMismatch: true, allowed: "GET" };
  }
  return {
    method,
    runtimePath: `/executive-authority/canonical-execution/missions/${missionId}`,
  };
}
...
function boundedGatewayRequestId(request) {
  const supplied = String(request.headers["x-request-id"] ?? "").trim();
  return STABLE_ID_PATTERN.test(supplied) ? supplied : randomUUID();
}

async function handleRealtimeCall(request, response, config, sessionAuthority) {
  const url = new URL(request.url, "http://portal.invalid");
  ...
  const requestId = boundedGatewayRequestId(request);
  structuredLog("experience_gateway_realtime_quarantined", {
    route: url.pathname,
    requestId,
    profile: COMMAND_PORTAL_REALTIME_PROFILE,
  });
  return sendJson(response, 503, {
    ok: false,
    data: null,
    error: {
      code: COMMAND_PORTAL_REALTIME_QUARANTINE_CODE,
      message: COMMAND_PORTAL_REALTIME_QUARANTINE_MESSAGE,
      requestId,
    },
    voice: {
      profile: COMMAND_PORTAL_REALTIME_PROFILE,
      activationAllowed: false,
      provesFullDuplexReadiness: COMMAND_PORTAL_PROVES_FULL_DUPLEX_READINESS,
...
export function createPortalServer(options = {}) {
  ...
  const server = createServer((request, response) => {
    ...
    if (retiredInteractionRoute(pathname, request.method)) {
    ...
    if (request.url?.startsWith("/api/auth")) {
      ...
      handleRealtimeCall(request, response, config, sessionAuthority)
