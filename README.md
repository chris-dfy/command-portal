...
Tenant Operational Context is also Runtime-owned. When the Command Portal-only `NEXUS_CONTEXT_ASSERTION_COMMAND_PORTAL_SECRET` is configured on this gateway and Runtime, the Experience Gateway issues a 60-second, single-use assertion under fixed key ID `context-assertion-command-portal-v1` and sends it on canonical interactions and Realtime voice transport requests. The Portal never falls back to a shared client assertion key. Browser metadata cannot select, replace, or strengthen a tenant profile. All browser-originated domain actions use the single canonical interaction ingress and require the signed operational session and its CSRF proof; Realtime is transcription transport only and finalized transcripts re-enter that ingress. Runtime verifies the assertion and assembles the same eight-domain context model for every client.
...
- Realtime voice is pinned to `nexus.realtime-voice@2.0.0` with the exact `trackless-pcm-transcription-v1` client profile. Before microphone activation, the browser and Experience Gateway require a compatible authenticated Runtime status. The browser captures one live microphone, proves analyser activity, and sends ordered 24 kHz PCM append/commit events over the WebRTC data channel; its SDP contains one inactive, trackless audio compatibility section and never requests or attaches provider output. Any provider output or server-VAD event is a protocol violation. Each finalized transcript enters `POST /executive/interactions` exactly once, and only the returned Runtime `response_text` may be narrated through local browser speech.

Runtime reachability, Runtime readiness, voice-contract readiness, provider connectivity, production readiness, and a live connection are independent states. The UI may display `Live Voice Active` only after `connectionState=live` and the current activation has verified a connected peer, open ordered data channel, live microphone track, PCM sample, analyser frame, exact negotiated contract/profile, immutable artifact identity, and current-session Realtime receipt digest. Historical receipts and the continuity push-to-talk path never satisfy that claim.
