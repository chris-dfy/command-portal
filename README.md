...
- Realtime voice uses WebRTC only for microphone input and finalized transcription. The browser advertises a send-only media direction, never requests or attaches provider output, treats any provider response/output event as a protocol violation, submits each finalized transcript exactly once through `POST /executive/interactions`, and may narrate only the returned Runtime `response_text` through local browser speech.
