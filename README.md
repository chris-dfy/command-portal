...
Tenant Operational Context is also Runtime-owned. When `NEXUS_CONTEXT_ASSERTION_SECRET` is configured on both services, the Experience Gateway issues a 60-second, single-use assertion for the fixed provisioned tenant and sends it on text and Realtime voice requests. Browser metadata cannot select, replace, or strengthen a tenant profile. Hosted text and Realtime voice mutations require the signed operational session and its CSRF proof. Runtime verifies the assertion and assembles the same eight-domain context model for every client.
