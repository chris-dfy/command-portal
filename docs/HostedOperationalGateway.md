...
Realtime SDP negotiation remains an exact `/api/runtime/realtime/call` transport route and requires the same signed session and CSRF proof. The Portal admits that transport only when Runtime reports the exact manual-commit contract (`serverVAD=false`, `clientAudioCommitRequired=true`, and `inputAudioCommitEvent=input_audio_buffer.commit`). A bounded analyser-driven speech envelope sends one audio-buffer commit per detected turn; the resulting committed provider item is bound to that turn's one idempotency key. Finalized voice transcripts do not use the transport as a reasoning side channel; they re-enter through `/executive/interactions`, and only Runtime response text may be narrated. Server-VAD and provider-response events fail closed. Session and provider-auth POSTs remain identity lifecycle only. Browser-selected attachment bytes remain browser-local until a governed canonical attachment transport is defined; a requested intake cannot be represented as completed without Runtime proof, receipt, and verified postconditions.
