...
Realtime SDP negotiation remains an exact `/api/runtime/realtime/call` transport route and requires the same signed session and CSRF proof. Finalized voice transcripts do not use that transport as a reasoning side channel; they re-enter through `/executive/interactions`. Session and provider-auth POSTs remain identity lifecycle only. Browser-selected attachment bytes remain browser-local until a governed canonical attachment transport is defined; a requested intake cannot be represented as completed without Runtime proof, receipt, and verified postconditions.
